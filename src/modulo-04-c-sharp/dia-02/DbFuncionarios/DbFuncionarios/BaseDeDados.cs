...
namespace DbFuncionarios
{
    public class BaseDeDados
    {
        ...
        // Exercicio A - Método que retorna uma lista de Funcionarios ordenado pelo nome do cargo
        public IList<Funcionario> OrdenadosPorCategoria()
        {
            return Funcionarios.OrderBy(funcionario => funcionario.Cargo.Titulo).ToList();
        }

        // Exercicio B - Método que retorna uma lista de funcionarios que contenham o parametro informado e ordenado pelo nome
        ...
        // Exercicio C - Método que retornar uma lista de objetos anonimos apenas com nome e titulo do cargo
        public IList<dynamic> BuscaRapida()
        {
            dynamic query = from funcionario in Funcionarios
                        select new
                        {
        ...
        // Exercicio D - Método sobrecarregado que retorna uma lista de funcionarios que contenham o parametro informado
        public IList<Funcionario> BuscarPorTurno(TurnoTrabalho turno1, TurnoTrabalho turno2, TurnoTrabalho turno3)
        {
            return Funcionarios.Where(funcionario => funcionario.TurnoTrabalho == turno1 || funcionario.TurnoTrabalho == turno2 || funcionario.TurnoTrabalho == turno3).ToList();
        }

        public IList<Funcionario> BuscarPorTurno(TurnoTrabalho turno1, TurnoTrabalho turno2)
        {
            return Funcionarios.Where(funcionario => funcionario.TurnoTrabalho == turno1 || funcionario.TurnoTrabalho == turno2).ToList();
        }
        ...
        public IList<dynamic> QtdFuncionariosPorTurno()
        {
            dynamic query = from funcionario in Funcionarios
                            ...
                            {
                                TituloCargo = funcionario.Cargo.Titulo,
                                QuantidadeDeFuncionarios = Funcionarios.Count()
                            };
            return query;
        }
    }
